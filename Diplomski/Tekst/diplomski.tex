...
Algoritmi koji su implementirani za ovaj problem podijeljeni su u dva dijela. Prva skupina su pohlepni algoritmi, a drugi algoritmi s kojima se i rad bavi su meta heuristike. Proučavaju se tri metaheuristike: agentski algoritam, genetski algoritam i genetsko kaljenje. Započinje se s konstruiranjem hiper-heuristike nad pohlepnim algoritmima, gdje se želi pokazati koji pohlepni algoritam je najbolji u raznim ovisnostima.
...
Agentski algoritam je direktno povezan s idejom pohlepnog algoritma. Ideja je da postoje agenti koji šeću grafom i u svakoj iteraciji čvoru na kojem se nalaze daju neku boju. U svakom koraku svaki agent nekom čvoru pridruži neku boju (ili ostavi istu). Svaki od agenata se boduje. Agenti s više bodova su prioritetniji i oni prvi pridružuju boje. To je ekvivalentno pohlepnom bojanju, gdje su sad čvorovi sortirani prema bodovima agenata. Dodatak na pohlepni algoritam je to što svaki agent nakoj globalne iteracije odlučuje što će napraviti. Može napraviti dvije vrste akcije: ostali na trenutnom čvoru ili se pomaknuti na neki susjedni. Odluka za akciju donosi se stohastički. Agent može otići na neki čvor ako \emph{vidi} priliku za lokalnom minimizacijom ili odabrati slučajan čvor. Postupak se provodi zadani broj globalnih iteracija. Unutar upravljačkog algoritma postoji mehanizam koji ako primjeti da postoji konvergencija u lokalni minimum poziva pohlepni algoritam da pomakne trenutno stanje u negu drugu točku prostora pretrage.

\subsection{Parametri algoritma}

\begin{enumerate}
	\item Broj agenata
	\item Broj globalnih iteracija
	\item Način na koji agenti bojaju čvorove (vrsta bojanja)
	\item Pohlepni algoritam - izlaz iz lokalnog minimuma
	\item Vjerojatnost agenta da ostane na istom čvoru
	\item Vjerojatnost da se agent slučajno pomakne na nekog susjeda
	\item Vjerojatnost da agent odabere susjeda prema kriteriju
\end{enumerate}

\subsection{Detalji algoritma}

Globale iteracije - sto je
Sto svaki agent napravi kako napravi kako se bira - malo probati kod
